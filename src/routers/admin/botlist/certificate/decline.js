...
console.log("[bhbotlist.xyz]: Admin/Botlist/Certificate Decline router loaded.");
...
app.post("/admin/certificate/delete/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has not been granted a certificate by <@${req.user.id}>\nReason: **${rBody['reason']}**`)
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}** certificate application has been declined by <@${req.user.id}>\nReason: **${rBody['reason']}**`)
    });
