...
console.log("[bhbotlist.xyz]: Admin/Botlist/Certificate Give router loaded.");
...
app.get("/admin/certificate/give/:botID", global.checkAuth, async (req, res) => {
    ...
    client.users.fetch(botdata.botID).then(bota => {
        client.channels.cache.get(channels.botlog).send(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate by <@${req.user.id}>`)
        client.users.cache.get(botdata.ownerID).send(`Your bot named **${bota.tag}** has been certified by <@${req.user.id}>`)
    });
